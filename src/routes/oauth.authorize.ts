...
import { isValidStaticToken } from '#/utils/mcp-handler'
import { createAuthorizationCode, getClient } from '#/utils/oauth-store'
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (typeof token !== 'string' || !isValidStaticToken(token)) {
